...
| **Ask** | A plain-English question box (⌘J) about spending, goals, or bills, answered from an aggregated summary of your own budget data — never raw account numbers. This screen ships with no backend of its own here (unlike the private Cross-Household- app it was ported from): answering the question means proxying it to the Claude API, which needs a small server route this static-hosted template doesn't include, so out of the box it always falls back to the local insights list. Wiring up your own proxy (a Cloudflare Worker, a Vercel/Netlify function, anything that can hold an API key server-side) turns on live answers and the weekly recap card on the Executive Summary; the fallback keeps the screen useful either way |
